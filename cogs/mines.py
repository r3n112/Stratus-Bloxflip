...
class Mines(discord.Cog):
    ...
    @discord.slash_command(guild_ids=[1012467016663322664])
    async def mines(self, ctx, round_id):
        ...
        if round_length <36 or round_length > 36:
            ...
            if path.is_file():
                ...
                with io.open(f'/home/container/data.json', 'w', encoding='utf8') as outfile:
                    str_ = json.dumps(usestype,
                                      indent=4, sort_keys=True,
                                      separators=(',', ': '), ensure_ascii=False)
                    outfile.write(to_unicode(str_))
